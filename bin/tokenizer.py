                ...
                if args.text_extractor:
                    if args.prefix == "baker" and args.text_extractor == "labeled_pinyin":
                        for c in tqdm(cut_set):
                            # Avoid duplicate tokens by checking if tokens already exist
                            if "tokens" not in c.supervisions[0].custom:
                        ...
                        for c in tqdm(cut_set):
                            if args.prefix == "cvd":
                                for supervision in c.supervisions:
                                    ...
                                    if "tokens" not in supervision.custom:
                                        phonemes = tokenize_text(
                                            text_tokenizer, text=supervision.text
                                        )
